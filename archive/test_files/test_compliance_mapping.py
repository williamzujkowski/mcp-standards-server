...
class ComplianceMappingTester:
    ...
    async def test_case(self, test_case: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if result and "result" in result:
                ...
                test_result["control_families"] = list(set(m.get("control_id", "").split("-")[0] for m in mappings if m.get("control_id")))
                test_result["standards_covered"] = list(set(m.get("standard_id") for m in mappings))
        ...
        try:
            ...
            if standards_result and "result" in standards_result:
                ...
                if result and "result" in result:
                    ...
                    test_result["mapping_count"] = len(mappings)
                    test_result["control_families"] = list(set(m.get("control_id", "").split("-")[0] for m in mappings if m.get("control_id")))
                    test_result["standards_with_mappings"] = list(set(m.get("standard_id") for m in mappings))
    ...
    async def generate_report(self):
        ...
        print(f"   • Control families: {sorted(list(all_control_families))}")
        ...
        report_data = {
            ...
            "coverage": {
                "control_families": sorted(list(all_control_families)),
                "standards_with_mappings": sorted(list(all_standards_with_mappings))
            },
