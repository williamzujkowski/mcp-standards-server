...
class BenchmarkVisualizer:
    ...
    def plot_timing_distribution(
        ...
        ax1.axvline(
            ...
            color="red",
            linestyle="--",
            label=f"Mean: {result.mean_time:.4f}s",
        )
        ax1.axvline(
            result.median_time,
            color="green",
            linestyle="--",
            label=f"Median: {result.median_time:.4f}s",
        )
    ...
    def plot_comparison(
        ...
        for bar, mem in zip(bars, memories, strict=False):
            ax.text(
                bar.get_x() + bar.get_width() / 2,
                bar.get_height(),
                f"{mem:.1f}",
        ...
        for bar, cv in zip(bars, cvs, strict=False):
            ax.text(
                bar.get_x() + bar.get_width() / 2,
                bar.get_height(),
                f"{cv:.3f}",
        ...
        if result.percentiles:
            ...
            for p in percentiles:
                if p in result.percentiles:
                    ax.axhline(
                        result.percentiles[p], color="gray", linestyle="--", alpha=0.5
                    )
                    ax.text(
                        max(x) * 0.02,
    ...
    def create_dashboard(
        ...
        if len(results) > 1:
            ...
            times = [r.mean_time for r in results]
            ax4.bar(names, times, color="skyblue")
        ...
        if latest.percentiles:
            ...
            values = [
                latest.percentiles[p] * 1000 for p in percentiles
            ]  # Convert to ms
