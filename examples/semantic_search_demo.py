...
def demo_performance_optimization():
    ...
    documents = []
    topics = ["python", "javascript", "react", "angular", "vue", "testing", "security", "api", "database", "performance"]
...
def demo_async_search():
    ...
    async def async_demo():
        ...
        # Index documents
        documents = [
            ("async-1", "Asynchronous programming in Python with asyncio", {"lang": "python"}),
            ("async-2", "JavaScript promises and async/await patterns", {"lang": "javascript"}),
        ...
        results_list = await asyncio.gather(*[
            search.search_async(query, top_k=3) 
            for query in queries
        ])
        elapsed = time.time() - start
