...
class TestMCPStandardsServer:
    ...
    def config(self):
        ...
        return {
            ...
            "privacy": {
                "detect_pii": False  # Disable privacy for most tests
            },
    ...
    async def test_get_applicable_standards_success(self, server):
        ...
        result = await server._get_applicable_standards(context, include_resolution_details=False)
    ...
    async def test_get_applicable_standards_with_error(self, server):
        ...
        with pytest.raises(Exception) as exc_info:
            await server._get_applicable_standards(context, include_resolution_details=False)
    ...
    async def test_metrics_recording(self, server):
        ...
        server.rule_engine.evaluate = Mock(return_value={"resolved_standards": [], "evaluation_path": []})
    ...
    async def test_cross_reference_standards(self, server):
        ...
        server.cross_referencer.get_references = AsyncMock(return_value={
            "references": [
                {
                    "id": "related-standard",
    ...
    async def test_get_analytics(self, server):
        ...
        server.analytics.get_analytics = AsyncMock(return_value={
            "total_queries": 1000,
