...
from src.core.cache import RedisCache, CacheConfig, cache_result, invalidate_cache
from src.core.cache.integration import (
    CachedSemanticSearch, CachedStandardsEngine,
    CachedRuleEngine, CacheWarmer, CacheMetricsCollector
)
...
class TestCacheIntegrationComponents:
    ...
    async def test_cache_warmer(self, cache, mock_semantic_search):
        ...
        start_time = time.time()
        results = await cached_search.search("security", k=5)
        ...
        start_time = time.time()
        standard = await cached_engine.get_standard("ISO27001")
