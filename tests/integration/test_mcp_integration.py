...
class TestMCPEndToEnd:
    ...
    async def test_prompt_workflow(self, initialized_app):
        ...
        # List prompts
        from src.server import get_prompt, list_prompts
...
class TestErrorHandling:
    ...
    async def test_authentication_error_handling(self):
        ...
        # Test 1: No token
        with pytest.raises(Exception), client.websocket_connect("/mcp"):
            pass

        # Test 2: Invalid token
        with pytest.raises(Exception):
            with client.websocket_connect("/mcp?token=invalid-token"):
                pass

        # Test 3: Expired token
        expired_token = "eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJ0ZXN0IiwiZXhwIjoxNjAwMDAwMDAwfQ.test"
        with pytest.raises(Exception):
            with client.websocket_connect(f"/mcp?token={expired_token}"):
                pass
