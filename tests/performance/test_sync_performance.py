...
class TestMemoryEfficiency:
        ...
        with patch.object(performance_synchronizer, '_list_repository_files', return_value=files):
            with patch.object(performance_synchronizer, '_filter_files', return_value=files):
                with patch.object(performance_synchronizer, '_download_file', 
                    ...
                    result = await sync_with_monitoring()
                    memory_tracker.snapshot("after_sync")
    ...
    def test_metadata_memory_efficiency(self, performance_synchronizer):
        ...
        initial_size = sys.getsizeof(performance_synchronizer.file_metadata)
        ...
        final_size = sys.getsizeof(performance_synchronizer.file_metadata)
...
class TestNetworkPerformance:
    ...
    async def test_rate_limit_performance(self, performance_synchronizer):
        ...
        print(f"- Initial rate limit: 100")
        print(f"- Total duration: {duration:.2f}s")
        print(f"- Files synced: {len(result.synced_files)}")
