...
class TokenOptimizationTester:
                ...
                for format_type in StandardFormat:
                    ...
                    try:
                        ...
                        format_results[format_type.value] = {
                            ...
                            "sections_included": len(compression_result.sections_included),
                            "sections_excluded": len(compression_result.sections_excluded),
                            "processing_time": end_time - start_time,
                            "warnings": compression_result.warnings
                        }
                        
                        print(f"    {format_type.value}: {compression_result.compressed_tokens} tokens "
    ...
    def test_compression_ratios(self) -> Dict[str, Any]:
        ...
        for size, standard_file in self.test_standards.items():
            ...
            for format_type in [StandardFormat.FULL, StandardFormat.CONDENSED, 
                ...
                actual_ratio = 1 - (compression_result.compressed_tokens / 
                ...
                compression_analysis[format_type.value] = {
                    ...
                    "content_quality": {
                        ...
                        "avg_line_length": sum(len(line) for line in content_lines) / len(content_lines) if content_lines else 0
                    }
                }
                ...
                print(f"    Sections: {len(compression_result.sections_included)} included, "
        ...
        for format_name, min_ratio in criteria.items():
            ...
            print(f"  {format_name}: {avg_ratio:.1%} compression {status} (target: >{min_ratio:.0%})")
    ...
    def test_progressive_loading(self) -> Dict[str, Any]:
        ...
        for test_case in test_cases:
            ...
            for i, batch in enumerate(loading_plan):
                ...
                print(f"    Sections: {', '.join(section_id for section_id, _ in batch)}")
            ...
            results[test_case["name"]] = {
                ...
                "efficiency": {
                    "avg_batch_size": total_sections / total_batches if total_batches > 0 else 0,
    ...
    def test_multi_standard_optimization(self) -> Dict[str, Any]:
        ...
        for scenario_name, total_budget in budget_scenarios.items():
            ...
            for standard in standards:
                ...
                print(f"  {standard['id']}: {format_type.value} format, "
            ...
            scenario_results["token_allocation"] = {
                ...
                "budget_utilization": total_compressed / total_budget.available if total_budget.available > 0 else 0
            }
            ...
            print(f"    Total compression: {(1 - scenario_results['token_allocation']['overall_compression']):.1%}")
            print(f"    Budget utilization: {scenario_results['token_allocation']['budget_utilization']:.1%}")
    ...
    def test_performance_benchmarks(self) -> Dict[str, Any]:
        ...
        for size, standard_file in self.test_standards.items():
            ...
            for format_type in [StandardFormat.FULL, StandardFormat.CONDENSED, 
                ...
                format_timings[format_type.value] = {
                    ...
                    "tokens_per_second": result.original_tokens / avg_time if avg_time > 0 else 0
                }
            ...
            for fmt, timing in format_timings.items():
                print(f"    {fmt}: {timing['avg_time']*1000:.1f}ms "
        ...
        for batch_size in batch_sizes:
            ...
            print(f"  Batch size {batch_size}: {total_time:.2f}s total, "
        ...
        results["cache_performance"] = {
            ...
            "cache_efficiency": (1 - cache_hit_time/cache_miss_time) * 100 if cache_miss_time > 0 else 0
        }
    ...
    def generate_report(self) -> None:
            ...
            for budget_type, expected_format in correct_selections.items():
                for size_data in fv_results.values():
                    if budget_type in size_data:
                        ...
                        if actual == expected:
                            print(f"   ✓ {budget_type} budget correctly selects {expected}")
                        else:
                            print(f"   ✗ {budget_type} budget selected {actual}, expected {expected}")
                        break
        ...
        if "compression_ratios" in self.results:
            ...
            for size_data in cr_results.values():
                for format_name, format_data in size_data.items():
                    format_compressions[format_name].append(format_data["compression_ratio"])
        ...
        if "progressive_loading" in self.results:
            ...
            for test_name, test_data in pl_results.items():
                print(f"   - {test_name}: {test_data['total_batches']} batches, "
        ...
        if "multi_standard_optimization" in self.results:
            ...
            for scenario, scenario_data in ms_results.items():
                ...
                print(f"   - {scenario}: {(1-alloc['overall_compression']):.1%} compression, "
