...
async def analyze_standards_catalog():
    ...
    for category in sorted(by_category.keys()):
        ...
        for standard in standards:
            ...
            print(f"     Description: {standard.description[:100]}{'...' if len(standard.description) > 100 else ''}")
            print()
    ...
    test_queries = [
        ("Security/Authentication", "authentication security best practices", ["security", "auth", "authentication"]),
        ("Database Performance", "database optimization performance tuning", ["database", "performance", "optimization"]),
        ("Accessibility", "WCAG accessibility guidelines screen readers", ["accessibility", "wcag", "a11y"]),
        ("AI/ML", "machine learning model deployment mlops", ["ai", "ml", "machine learning", "mlops"]),
        ("React", "reactjs component patterns hooks", ["react", "javascript", "frontend"])
    ]
    ...
    for query_name, query, keywords in test_queries:
        ...
        if relevant_standards:
            print(f"   📈 Found {len(relevant_standards)} potentially relevant standards:")
            for standard, match_count in relevant_standards[:3]:  # Show top 3
                print(f"     • {standard.title} ({standard.category}) - {match_count} keyword matches")
        else:
    ...
    accessibility_standards = [s for s in all_standards if 'accessibility' in s.title.lower() or 'wcag' in s.title.lower() or 'a11y' in ' '.join(s.tags)]
    print(f"Accessibility standards: {len(accessibility_standards)}")
    
    ai_ml_standards = [s for s in all_standards if any(term in f"{s.title} {s.description}".lower() for term in ['ai', 'ml', 'machine learning', 'neural', 'model training'])]
    print(f"AI/ML standards: {len(ai_ml_standards)}")
    
    db_standards = [s for s in all_standards if any(term in f"{s.title} {s.description}".lower() for term in ['database', 'sql', 'nosql', 'db'])]
