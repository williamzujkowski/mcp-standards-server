...
class MemoryProfileAnalyzer:
    ...
    def analyze_file(self, filepath: str) -> Dict[str, any]:
        ...
        report = {
            ...
            'largest_allocation': max(allocations, key=lambda x: x[1]) if allocations else None,
        ...
        for location, data in location_totals.items():
            if data['size'] > self.threshold_mb:
                leaks.append({
                    ...
                    'descriptions': list(set(data['descriptions']))[:5]  # Top 5 unique
                })
...
def main():
    ...
    parser.add_argument(
        'profile_file',
        help='Path to memory profile file'
    )
    parser.add_argument(
        '--threshold',
        type=float,
